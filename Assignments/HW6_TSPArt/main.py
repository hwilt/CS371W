...
"""np.random.seed(0)
I = skimage.io.imread("images/henry_running.jpg")
# Initial stippling
...
np.random.seed(0)
I = skimage.io.imread("images/henry.jpg")
# Initial stippling
X = voronoi_stipple(I, thresh=0.3, target_points=2000, canny_sigma=0.8)
# Filter out lowest 4 points by density
X = density_filter(X, (X.shape[0]-4)/X.shape[0]) 
...
plt.scatter(X[tour, 0], X[tour, 1], s=15, c=np.arange(len(tour)), cmap='twilight')
plt.gca().set_facecolor((0.8, 0.8, 0.8))
plt.savefig("henry.jpg", bbox_inches='tight')
plt.show()